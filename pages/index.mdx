###

# Cozy Creators

Welcome to **Cozy Creators**! Your one stop shop for a generative-AI backend. Lovingly written in Python.

## What is Cozy Creators?

Cozy Creators is a generative-AI backend that allows you to generate images, text, and more. It is built on top of the
Diffusers architecture, and is designed to be easy to use and integrate into your projects.

## Why Cozy Creators?

- Cozy Creators is designed to be extremely easy to use and integrate into your projects.
- It is built on top of the Diffusers architecture, which is a powerful and flexible framework for running generative-AI
  models.
- Cozy Creators provides a simple and intuitive API that allows you to quickly and easily generate images, text, and
  more.
- It is also designed to be highly customizable, so you can easily tailor it to your specific needs.

export default ({ children }) => <div className="prose dark">{children}</div>;
